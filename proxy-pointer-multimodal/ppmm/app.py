...
Features
  * Drop PDF / DOCX files into the UI -> they are extracted, structured into a
    proxy-pointer tree, embedded, and indexed on the fly (OpenAI/Azure).
...
def _has_key() -> bool:
    return bool(config.OPENAI_API_KEY or config.AZURE_OPENAI_API_KEY)
...
with st.sidebar:
    ...
    if _has_key():
        st.success("OpenAI/Azure API key detected", icon="✅")
    else:
    ...
    if uploads and st.button("Process & index", type="primary", use_container_width=True):
        if not _has_key():
            st.error("An OpenAI or Azure key is required to embed & index documents.")
        else:
...
with tab_ask:
    ...
    if go:
        if not _has_key():
            st.error("Add an OpenAI or Azure API key first.")
