# Proxy-Pointer MultiModal (OpenAI/Azure version)
...
This build differs from the original in three ways:

1. **MultiModal only.** No Text-Only and no DocComparator.
2. **OpenAI / Azure OpenAI only — no Adobe.** The original used the Adobe Extract PDF API.
   This version extracts everything locally with **PyMuPDF** (PDF) and
   **python-docx** (DOCX), so an OpenAI or Azure key is all you need.
...
Embed nodes (text-embedding-3-large, 3072-d)  ──►  FAISS cosine index
                                                       │
Query ─► embed ─► recall k=200 ─► dedup ─► re-rank (GPT-4o, anchor-aware) ─► top 5
                                                       │
                       load full section text + real images
                                                       │
        GPT-4o multimodal synthesis ─► answer + [SHOW: img | caption]
                                                       │
...
cp .env.example .env
# edit .env -> OPENAI_API_KEY=... or AZURE_OPENAI_API_KEY=...
```
